...
inline Quaternion catmull_rom_spline(const Quaternion &q1, const Quaternion &q2,
		const Quaternion &q3, const Quaternion &q4, scalar_t t);
...
template <typename T>
TrackKey<T>::TrackKey(const T &v, double t)
{
	val = v;
...
T Track<T>::operator()(int time) const
{
	...
	case INTERP_LINEAR:
		if(end) {
			...
			double t = (double)(time - start->time) / (double)(end->time - start->time);
...
template <typename T>
T Track<T>::operator()(double t) const
{
	return (*this)((int)(HENGE_ANIM_KEY_FLT_SCALE * t));
}

// TODO implement higher order quaternion interpolation ?
inline Quaternion catmull_rom_spline(const Quaternion &q1, const Quaternion &q2,
		const Quaternion &q3, const Quaternion &q4, scalar_t t)
{
